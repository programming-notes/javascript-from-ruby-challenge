...
One of the selling points of Ruby, in its early days, was its robust `Enumerable` methods. In your time working with DBC you may have grown exceedingly fond of these methods and might be missing them presently: they're really handy! Responding to feedback from the polyglottal JavaScript development community, JavaScript `Array` has gained, among others, the methods `forEach`, `reduce`, and `filter`. While these offer much of the convenience of Ruby's `Enumerable`, there are a number of quirks that may require further investigation on your part. Check out the examples provided at MDN and try to update the preceding code to take advantage of these methods. What might the trade-offs be? What's clearer? What's more opaque? Examine the [Array.prototype](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array) documentation.
